...
class LoginManager {
  ...
  async fillPhoneAndNext() {
    ...
    // Live UI confirmed: button text is "Continue" (not "Next")
  ...
  async waitForManualOtpSuccess() {
    ...
    logger.info(`🔐 Manual OTP required for ${this.account?.id || 'default account'}.
    Complete login within ${config.website.otpWaitMinutes} minutes.`);
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      if (state === 'logged_in' || (iteration % 3 === 0 && await this.isLoggedIn())) {
        ...
        logger.info('✅ Login successful. Session files saved.');
        return true;
      }
