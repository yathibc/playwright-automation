...
class EventMonitor {
  ...
  async checkForNewContent() {
    try {
      const hasBookingElements = await this.browser.page.locator('button:has-text("Book"), button:has-text("Ticket"), .book-now').count();
      const hasSeatMap = await this.browser.page.locator('svg, [data-seat], canvas').count();
      
      return hasBookingElements > 0 || hasSeatMap > 0;
    } catch (error) {
      return false;
    }
  }
