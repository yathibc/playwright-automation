...
async function runTests() {
  ...
  try {
    ...
    // Test 3: Telegram notification (if configured)
    logger.info('Test 3: Telegram notification');
    const TelegramNotifier = require('../notifications/telegram');
    const telegram = new TelegramNotifier();
    await telegram.sendMessage('🧪 Test Message - System is working correctly');
    logger.info('✅ Telegram notification sent');
    
    // Test 4: Browser initialization (single session)
    ...
    logger.info('Use direct module validation / manual execution when OTP capacity is available; avoid running npm start if OTP is rate-limited.');
